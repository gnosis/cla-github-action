...
async function getComment() {
  try {
    ...
    return response.data.find((comment) =>
      comment.body.match(/.*CLA Assistant Lite.*/)
    )
  } catch (e) {
    core.setFailed(
      `Error occured when getting  all the comments of the pull request: ${e.message}`
    )
  }
}
...
function commentContent(signed: boolean, committerMap: CommitterMap): string {
  ...
  ***I have read the CLA Document and I hereby sign the CLA***
  ...
  if (
    ...
    }** committers have signed the CLA.`
  ...
  if (committerMap && committerMap.unknown && committerMap.unknown.length > 0) {
    ...
    text += ` - - -
    **${committerNames.join(", ")}** ${seem} not to be a GitHub user.`
    text +=
